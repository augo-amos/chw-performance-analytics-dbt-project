...
#}
    ...
    CASE 
        -- Checking if day of month is >= 26 (Snowflake -- DAY() function)
        WHEN DAY({{ date_column }}) >= 26 THEN
            -- If yes: shift to next month, then truncate to first day
            DATE_TRUNC('MONTH', DATEADD(MONTH, 1, {{ date_column }}))
        ELSE
            -- If no: truncate current month to first day
